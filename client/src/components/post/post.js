...
const Post = (props) => {
  ...
  const [userId, setUserId] = useState('609392f8efa1ad0d148661e7');
  // const [userId, setUserId] = useState(props.match.params.userId);

  // const postUrl = 'http://localhost:8080/api/posts/upload-post' + props.match.params.userId;

  //TODO : SEND PARAMS.USERID AND DEFINITELY NOT LIKE THIS
  const postUrl = 'http://localhost:8080/api/posts/upload-post/609392f8efa1ad0d148661e7';
  const onChange = (e) => {
  ...
  const onFinish = (values) => {
    setIsLoading(true);
    const data = { user_id: userId, content: values.contentText, link: linkVal, image: values.contentImage };
  ...
  return (
    <Form className='main-container' onFinish={onFinish}>
      ...
      <div className='post-holder'>
        ...
        <Form.Item className='input' name='contentText' rules={[{ required: true, message: 'Please add something before posting' }]}>
      ...
      <Form.Item name='contentImage' className={images.preview ? 'image-tab active' : 'image-tab'}>
      ...
      <div className={active ? 'post-bottom active' : 'post-bottom '}>
        <Form.Item>
          <Button className='publish-post-btn' htmlType='submit' type='primary' block style={{ borderRadius: '5px' }} loading={isLoading}>
            {'Publish'}
          </Button>
        </Form.Item>
      </div>
